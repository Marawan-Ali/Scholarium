...
namespace LibraryMangementSystem.Controllers
{
    ...
    public class LoansController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(LoanViewModel loanData, int selectedEmail, int selectedBook)
        {
            if (loanData != null)
            {
                var book = await database.Books.FirstOrDefaultAsync(s => s.BookID == selectedBook);
                var member = await database.Members.FirstOrDefaultAsync(s => s.MemberID == selectedEmail);
        ...
        [HttpGet]
        public async Task<IActionResult> SearchEmails(string searchEmail)
        {
            var emails = database.Members;

            var items = await emails
           .Where(i => i.Email.Contains(searchEmail))
           .Select(i => new { id = i.MemberID, text = i.Email })
           .ToListAsync();

            return Json(items);
        }
        [HttpGet]
        public async Task<IActionResult> SearchBooks(string searchBook)
        {
            var books = database.Books;

            var items = await books
           .Where(i => i.Title.Contains(searchBook))
           .Select(i => new { id = i.BookID, text = i.Title })
           .ToListAsync();
        ...
        public async Task<IActionResult> SearchByTitle(string bookTitle, int? page)
        {
            ...
            if (bookTitle != null)
            {
                loans = await database.Loans.Where(b => b.Book.Title.Contains(bookTitle)).ToListAsync();
            }
            int totalItems = loans.Count();
            int totalPages = (int)Math.Ceiling((double)totalItems / PageSize);
            ViewBag.TotalPages = totalPages;
        ...
        public async Task<IActionResult> SearchByMember(string member, int? page)
        {
            ...
            if (member != null)
            {
                loans = await database.Loans.Where(b => b.Member.Email.Equals(member)).ToListAsync();
            }
            int totalItems = loans.Count();
            int totalPages = (int)Math.Ceiling((double)totalItems / PageSize);
            ViewBag.TotalPages = totalPages;
        ...
        public async Task<IActionResult> SearchByLoanDate(DateTime startdate, DateTime enddate, int? page)
        {
            ...
            {
                loans = await database.Loans.Where(d => d.LoanDate.Day >= startdate.Day && d.LoanDate.Day <= enddate.Day).ToListAsync();
            }
            int totalItems = loans.Count();
            int totalPages = (int)Math.Ceiling((double)totalItems / PageSize);
            ViewBag.TotalPages = totalPages;
        ...
        public async Task<IActionResult> SearchByDueDate(DateTime startdate, DateTime enddate, int? page)
        {
            ...
            if (startdate != null || enddate != null)
            {
                loans = await database.Loans.Where(d => d.DueDate.Day >= startdate.Day && d.DueDate.Day <= enddate.Day).ToListAsync();
            }
            int totalItems = loans.Count();
            int totalPages = (int)Math.Ceiling((double)totalItems / PageSize);
            ViewBag.TotalPages = totalPages;
